...
const RideResultCard: React.FC<RideResultCardProps> = ({ ride }) => {
  ...
  return (
    <div className={cn("block group font-sans relative", isPast && "opacity-75 select-none")}>
      <Link href={isPast ? "#" : `/rides/${ride.id}`} className={cn("block", isPast && "cursor-default pointer-events-none")}>
        <div className={cn(
          "premium-card p-5 md:p-6 transition-all duration-300 bg-white border-2",
          isPast ? "border-gray-100 grayscale-[0.2]" : "hover:border-primary hover:shadow-2xl hover:shadow-primary/5 border-transparent"
        )}>
          {isPast && (
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="grow min-w-0">
              ...
              <div className="flex items-start space-x-4 mb-6">
                <div className="flex flex-col items-center self-stretch py-1 shrink-0">
                  <div className={cn("w-2.5 h-2.5 rounded-full border-2 bg-white", isPast ? "border-gray-400" : "border-primary")}></div>
                  <div className="flex-1 w-px border-l border-dashed border-border group-hover:border-primary/40 transition-colors my-1"></div>
                  <div className={cn("w-2.5 h-2.5 rounded-full border-2 bg-white", isPast ? "border-gray-300" : "border-secondary")}></div>
                </div>
                ...
                <div className="flex-1 min-w-0 space-y-4">
                  ...
                  <div>
                    <div className="flex items-center text-[9px] font-black text-gray-400 uppercase tracking-widest mb-0.5">
                      ...
                      <span className={cn("normal-case tracking-normal", isPast ? "text-gray-500" : "text-primary")}>
              ...
              <div className="flex flex-wrap items-center gap-6">
                <div className="flex items-center">
                  <div className={cn(
                    ...
                    isPast ? "bg-gray-100 text-gray-400 border-gray-200" : "bg-primary/10 text-primary border-primary/20"
                  )}>
                    {driverName.charAt(0)}
                  </div>
                  <div>
            ...
            <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center gap-4 shrink-0">
              ...
              >
                {isPast ? t("status", "past") || "PAST" : t("rides", "joinRide")}
              </Button>
            </div>
          </div>
        </div>
