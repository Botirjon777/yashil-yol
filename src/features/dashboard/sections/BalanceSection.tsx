...
export function BalanceSection({
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
        ...
        <div className="premium-card p-5 lg:p-8">
          <div className="flex items-center justify-between mb-8">
            <h3 className="font-black text-dark-text text-lg">
              {t("dashboard", "balance")?.savedCards}
            </h3>
            <button
          ...
          <div className="space-y-4">
            {isLoading ? (
              ...
              cards.map((card) => (
                <div
                  key={card.id}
                  className="flex items-center justify-between p-2.5 lg:p-4 bg-light-bg rounded-2xl border border-border group/card"
                >
                        ...
                        {card.status === "not_verified" && (
                          <div className="flex items-center gap-2">
                            <span className="bg-warning/10 text-warning px-1.5 py-0.5 rounded-md text-[8px]">
                              {t("dashboard", "balance")?.notVerified ||
                                "NOT VERIFIED"}
                            </span>
                            <button
              ...
              <p className="text-gray-400 font-medium text-center py-4 text-sm italic">
                {t("dashboard", "balance")?.noCards}
              </p>
            )}
          </div>
        </div>
      ...
      {/* Withdrawals History */}
      <div className="premium-card p-5 lg:p-8 mt-8">
        <h3 className="font-black text-dark-text text-lg mb-6">
          {t("dashboard", "balance")?.withdrawals || "Withdrawals"}
        </h3>
        ...
        <div className="space-y-4">
          {isWithdrawalsLoading ? (
            ...
            withdrawals.map((w: any) => (
              <div
                key={w.id}
                className="flex items-center justify-between p-4 bg-light-bg rounded-2xl border border-border"
              >
                <div className="flex items-center space-x-4">
                  <div
            ...
            <p className="text-gray-400 font-medium text-center py-4 text-sm italic">
              {t("dashboard", "balance")?.noWithdrawals || "No withdrawals yet"}
            </p>
          )}
        </div>
      </div>
